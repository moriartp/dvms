...
function renderChart(dataset){
  ...
  var width = 855 - margin.left - margin.right,
      ...
      radius = function(d){ return Math.sqrt(d.alcoholfrequency)*3 }


  //// make scales with d3.extent()

  var xExtent = d3.extent( dataset.map( function(d){ return parseInt(d.ageMidpoint) }) )
  var yExtent = d3.extent( dataset.map( function(d){ console.log(d.alcoholuse);
    return parseFloat(d.alcoholuse) }) )
...
function renderChart(dataset){
  ...
  var svg = d3.select('#svg-wrapper-2').append('svg')
    .attr('id', 'svg')
    .attr('width', width + margin.left + margin.right)
    .attr('height', height + margin.top + margin.bottom)
